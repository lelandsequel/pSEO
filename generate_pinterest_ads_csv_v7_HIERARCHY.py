...
campaign_row['Daily Spend Limit'] = '1000000'  # Campaign level budget cap (optional but good)
rows.append(campaign_row)

# --- ROW 2: AD GROUP ---
adgroup_row = {col: '' for col in columns}
adgroup_row['Action'] = 'CREATE'
adgroup_row['Campaign Name'] = 'PSEO Traffic Gen'  # Links to Campaign
...
adgroup_row['Ad Group Budget'] = '1000000'  # $10.00
...
for i in range(1, 51):
    ...
    pin_row['Ad Group Name'] = 'PSEO Pins'  # Links to Ad Group
    ...
    pin_row['Promoted Pin URL'] = f"https://www.applianceerror.com/{slug}"
...
for i in range(1, 51):
    ...
    pin_row['Promoted Pin URL'] = f"https://www.errorcodehelp.com/{slug}"
    pin_row['Ad Format'] = 'IMAGE'
    pin_row['Promoted Pin Status'] = 'ACTIVE'
    pin_row['Is Ad-only Pin'] = 'FALSE'
    rows.append(pin_row)
...
for i in range(1, 51):
    ...
    pin_row['Promoted Pin URL'] = f"https://www.saasguide.org/{slug}"
    pin_row['Ad Format'] = 'IMAGE'
    pin_row['Promoted Pin Status'] = 'ACTIVE'
    pin_row['Is Ad-only Pin'] = 'FALSE'
    rows.append(pin_row)
...
output_file = 'pinterest_bulk_upload_v7_HIERARCHY.csv'
df_final.to_csv(output_file, index=False)

print(f"✅ Created {output_file}")
print(f"   - Structure: Hierarchical (Campaign -> Ad Group -> Pins)")
print(f"   - Total Rows: {len(df_final)} (1 Campaign + 1 Ad Group + 150 Pins)")
print(f"   - Solves Error 2033: 'The row contains fields from multiple entities'")

# 5. Check Image Accessibility (Sample)
print("\n🔍 Checking image accessibility...")
sample_url = rows[2]['Media File Name'] # First pin row
try:
