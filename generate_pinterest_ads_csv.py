...
# 1. Define the Full Column List (From your template)
# This ensures Pinterest accepts the format 100%.
template_columns = [
...
# 3. Create DataFrame & Map Data
df_final = pd.DataFrame(columns=template_columns)
df_source = pd.DataFrame(data)

# Map the generated data to the template columns
...
# 4. Fill Mandatory Ad Fields
df_final['Campaign Name'] = "PSEO Traffic Gen"
df_final['Campaign Objective'] = "AWARENESS"
df_final['Campaign Status'] = "PAUSED"
df_final['Ad Group Name'] = "PSEO Pins"
df_final['Ad Group Status'] = "PAUSED"
df_final['Ad Group Budget Type'] = "DAILY"
df_final['Ad Group Budget'] = "1000000"  # $10.00
df_final['Bid strategy type'] = "AUTOMATIC_BID"
df_final['Ad Format'] = "IMAGE"

# 5. Export
df_final.to_csv('pinterest_bulk_upload_formatted.csv', index=False)
print("✅ Success! 'pinterest_bulk_upload_formatted.csv' created.")
print(f"   📊 Total pins: {len(df_final)}")
print(f"   📋 Total columns: {len(template_columns)}")
