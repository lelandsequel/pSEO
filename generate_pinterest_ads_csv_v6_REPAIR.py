...
# 2. Define Columns (Action + Template)
columns = [
...
for i in range(1, 51):
    ...
    data.append({
        ...
        "Promoted Pin URL": f"https://www.applianceerror.com/{slug}",
        "Media File Name": f"https://www.applianceerror.com/social/{slug}.png"
    })
...
for i in range(1, 51):
    ...
    data.append({
        ...
        "Promoted Pin URL": f"https://www.errorcodehelp.com/{slug}",
        "Media File Name": f"https://www.errorcodehelp.com/social/{slug}.png"
    })
...
for i in range(1, 51):
    ...
    data.append({
        ...
        "Promoted Pin URL": f"https://www.saasguide.org/{slug}",
        "Media File Name": f"https://www.saasguide.org/social/{slug}.png"
    })
...
df_final['Ad Group Budget'] = "1000000" # 1.00 USD
df_final['Bid strategy type'] = "AUTOMATIC_BID"
df_final['Ad Format'] = "IMAGE"
df_final['Promoted Pin Status'] = "ACTIVE"
df_final['Is Ad-only Pin'] = "FALSE"

# 5. Export
output_file = 'pinterest_bulk_upload_v6_REPAIR.csv'
df_final.to_csv(output_file, index=False)

print(f"✅ Created {output_file}")
print(f"   - Updated URLs to use 'www' (Canonical)")
print(f"   - Verified Action='CREATE' column")
print(f"   - Total Pins: {len(df_final)}")

# 6. Check Image Accessibility (Sample)
print("\n🔍 Checking image accessibility...")
sample_url = df_final['Media File Name'].iloc[0]
try:
