...
# 1. Get Today's Date in MM/DD/YYYY format (Pinterest Requirement)
...
# Changed to CONSIDERATION (Traffic) as AWARENESS often has stricter bid requirements
campaign_row['Campaign Objective'] = 'CONSIDERATION' 
campaign_row['Campaign Status'] = 'ACTIVE'
campaign_row['Campaign Start Date'] = today # MM/DD/YYYY
campaign_row['Campaign Start Time'] = '00:00:00' # HH:MM:SS
...
adgroup_row['Ad Group Start Date*'] = today # MM/DD/YYYY
adgroup_row['Ad Group Start Time'] = '00:00:00' # HH:MM:SS
...
for i in range(1, 51):
    ...
    pin_row['Organic Pin URL'] = url
    pin_row['Image Alternative Text'] = f"Fix Whirlpool Error F{i:02d}"
    pin_row['Ad Format'] = 'image' # Lowercase 'image' often fixes validation
    pin_row['Promoted Pin Status'] = 'ACTIVE'
    pin_row['Is Ad-only Pin'] = 'False' # Title Case 'False'
    rows.append(pin_row)
...
for i in range(1, 51):
    ...
    pin_row['Promoted Pin URL'] = url
    pin_row['Organic Pin URL'] = url
    pin_row['Image Alternative Text'] = f"Fix OBD2 Code P{i:04d}"
...
for i in range(1, 51):
    ...
    pin_row['Promoted Pin URL'] = url
    pin_row['Organic Pin URL'] = url
    pin_row['Image Alternative Text'] = f"Tool {i} vs Tool {i+1} Comparison"
    pin_row['Ad Format'] = 'image'
    pin_row['Promoted Pin Status'] = 'ACTIVE'
    pin_row['Is Ad-only Pin'] = 'False'
    rows.append(pin_row)
...
output_file = 'pinterest_bulk_upload_v9_FORMATS.csv'
df_final.to_csv(output_file, index=False)

print(f"✅ Created {output_file}")
print(f"   - Fixed Date Format: MM/DD/YYYY (e.g., {today})")
print(f"   - Fixed Time Format: HH:MM:SS")
print(f"   - Fixed Boolean: 'False' (Title Case)")
print(f"   - Fixed Ad Format: 'image' (lowercase)")
print(f"   - Changed Objective: CONSIDERATION (Traffic)")
