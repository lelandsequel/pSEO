...
# 2. Define Columns
# We take the columns from the user's template and PREPEND 'Action'.
# This is critical because the error explicitly asks for an 'Action' column.
template_columns = [
...
final_columns = ['Action'] + template_columns
...
# 5. Map Data
df_final['Action'] = "CREATE"  # <--- THE FIX
...
df_final['Ad Group Budget'] = "1000000"  # $10.00 USD
...
# 7. Save as V4
df_final.to_csv('pinterest_bulk_upload_v4_FINAL.csv', index=False)
print("✅ Created pinterest_bulk_upload_v4_FINAL.csv")
print(f"   📅 Start Date: {today}")
print(f"   💰 Budget: $10.00/day")
print(f"   📊 Pins: {len(df_final)}")
print(f"   🔧 Action: CREATE")
print(f"   📋 Columns: {len(final_columns)}")
print("\n🎯 FINAL VERSION - Ready for Pinterest upload!")
